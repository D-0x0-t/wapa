...
import string
import sys
from netifaces import interfaces # not working on windows???
import cmd
...
class WapaShell(cmd.Cmd):
    ...
    def do_man(self, args):
        """
        AGREGAR MANUALES DE USO PARA LOS COMANDOS, prob incluir una web con github.io?
        """
    ...
    def do_fuzz(self, args):
        ...
        """
        try: # añadir los fuzzer de disas y deauth
            args = args.split()
            mode = str(args[0])
            if mode.lower() == "start":
...
if __name__ == '__main__':
    ...
    # interface daemon
    # maybe add it as a command and warn the user if wid interferes with the shell execution
    # podría modificar el daemon para que compruebe el estado pero en vez de modificarlo, que simplemente lo actualice en la bbdd, complicación por ifaces virtuales...
    ...
    # Preinitialize modules // multilpe errors, leave it
    #network_scanner = WAPANetworkScanner()
